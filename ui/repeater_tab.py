...
class _RepeaterPane (ttk .Frame ):
    ...
    def _build (self ):
        ...
        ttk.Button(bar, text="Save Request…",   command=self._save_request).pack(side="left", padx=2)
        ttk.Button(bar, text="Save Response…",  command=self._save_response).pack(side="left", padx=2)
        ttk.Button(bar, text="Save Both…",      command=self._save_both).pack(side="left", padx=2)

        self._status_var = tk.StringVar()
        ttk.Label(bar, textvariable=self._status_var, foreground="gray").pack(side="left", padx=8)
    ...
    def _save_request(self):
        ...
        if path:
            with open(path, "w", encoding="utf-8") as f:
                f.write(self._req_text.get("1.0", "end-1c"))

    def _save_response(self):
        content = self._resp_text.get("1.0", "end-1c")
        if not content.strip():
            messagebox.showwarning("No Response", "Send a request first.")
            return
        path = filedialog.asksaveasfilename(
            title="Save Response",
            defaultextension=".txt",
    ...
    def _save_both(self):
        ...
        if path:
            with open(path, "w", encoding="utf-8") as f:
                ...
                f.write(self._req_text.get("1.0", "end-1c"))
                f.write("\n\n" + "=" * 60 + "\nRESPONSE\n" + "=" * 60 + "\n")
                resp = self._resp_text.get("1.0", "end-1c")
                f.write(resp if resp.strip() else "(no response)")
