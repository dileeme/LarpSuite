...
def launch_proxied_browser (port :int ):
    ...
    profile_dir =os .path .join (os .environ .get ("TEMP","C:\\Temp"),"larpsuite_browser_profile")
...
class ProxyTab (ttk .Frame ):
    ...
    def _build (self ):
        ...
        self ._req_text =tk .Text (req_frame ,wrap ="none",bg ="#0A0B10",fg ="#E8DCC8",
        ...
        self ._resp_text =tk .Text (resp_frame ,wrap ="none",bg ="#0A0B10",fg ="#E8DCC8",
        font =("Consolas",9 ),state ="disabled")
        ...
        bg ="#1A1500",fg ="#E8C870",font =("Consolas",9 ),
        )
    ...
    def _right_click (self ,event ):
        ...
        menu .add_separator ()
        menu .add_command (label ="Save Request…",      command =self ._save_request )
        menu .add_command (label ="Save Response…",     command =self ._save_response )
        menu .add_command (label ="Save Both…",         command =self ._save_both )
    ...
    def _save_request (self ):
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("Text", "*.txt"), ("HTTP", "*.http"), ("All", "*.*")],
            initialfile=f"request_{self._selected_entry.id}.txt",
        )
        if path:
            with open(path, "w", encoding="utf-8") as f:
                f.write(self._selected_entry.request.to_display())

    def _save_response (self ):
        if not self ._selected_entry or not self ._selected_entry .response :
            messagebox.showwarning("No Response", "No response available for this entry.")
            return
        path = filedialog.asksaveasfilename(
            title="Save Response",
            defaultextension=".txt",
            filetypes=[("Text", "*.txt"), ("HTTP", "*.http"), ("All", "*.*")],
            initialfile=f"response_{self._selected_entry.id}.txt",
        )
        if path:
            with open(path, "w", encoding="utf-8") as f:
                f.write(self._selected_entry.response.to_display())

    def _save_both (self ):
        if not self ._selected_entry :
            return
        path = filedialog.asksaveasfilename(
            title="Save Request + Response",
            defaultextension=".txt",
            filetypes=[("Text", "*.txt"), ("HTTP", "*.http"), ("All", "*.*")],
            initialfile=f"entry_{self._selected_entry.id}.txt",
        )
        if path:
            with open(path, "w", encoding="utf-8") as f:
                f.write("=" * 60 + "\n")
                ...
                f.write("RESPONSE\n")
                f.write("=" * 60 + "\n")
                if self._selected_entry.response:
                    f.write(self._selected_entry.response.to_display())
                else:
                    f.write("(no response)")
