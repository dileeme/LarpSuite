...
FIND_COLS =("File","Line","Severity","Confidence","Category","CWE","Snippet")
...
class DiagnosticsTab (ttk .Frame ):
    ...
    def _build (self ):
        ...
        ttk .Label (fbar ,text ="Severity:").pack (side ="left")
        ...
        ttk .Label (fbar ,text ="Confidence:").pack (side ="left")
        self ._conf_var =tk .StringVar (value ="All")
        ttk .Combobox (fbar ,textvariable =self ._conf_var ,
        values =["All","HIGH","MEDIUM","LOW"],
        width =8 ,state ="readonly").pack (side ="left",padx =4 )
        self ._conf_var .trace_add ("write",lambda *_ :self ._refresh_findings ())

        ttk .Label (fbar ,text ="Category:").pack (side ="left")
        ...
        selectmode ="browse")
        widths =[210 ,50 ,75 ,75 ,150 ,75 ,300 ]
    ...
    def _show_result (self ,result :AnalysisResult ):
        ...
        for conf in ("HIGH","MEDIUM","LOW"):
            if conf in conf_counts :
                lines .append (f"  {conf :<10} {conf_counts [conf ]:>4}")
    ...
    def _refresh_findings (self ):
        ...
        and (conf_flt =="All"or f .confidence ==conf_flt )
        and (cat_flt =="All"or f .category ==cat_flt )
        ]

        for f in visible :
            self ._tree .insert ("","end",tags =(f .severity ,),values =(
            f .file ,f .line ,f .severity ,f .confidence ,
            f .category ,f .cwe ,f .snippet ,
            ))
    ...
    def _on_select (self ,_event =None ):
        ...
        if not vals :
            return 
        file_ ,line ,sev ,conf ,cat ,cwe ,snippet =vals 

        detail_finding =next (
        (f for f in self ._findings 
        if f .file ==file_ and str (f .line )==str (line )and f .category ==cat ),
        None 
        )
        ...
        f"Severity   : {sev }   Confidence: {conf }   Category: {cat }{cwe_str }\n"
        f"Snippet    : {snippet }\n"
        f"Detail     : {detail }"
        )
    ...
    def _export (self ):
        ...
        lines =[
        "Larp Suite — Code Diagnostics Report",
        "="*60 ,
        ...
        if x .severity in _sev_order else 99 ):
            cwe_str =f"  {f .cwe }"if f .cwe else ""
            lines .append (
            f"[{f .severity :<8}] [Confidence:{f .confidence :<6}] "
            f"{f .category :<25}{cwe_str }\n"
