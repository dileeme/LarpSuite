"""
MITM Proxy — threaded HTTP/HTTPS intercepting proxy.

Flow:
  HTTP  → parse request → optional intercept → forward → record response
  HTTPS → CONNECT tunnel → wrap both sides in SSL → same as HTTP above
"""
...
class InterceptFlag:
    """Controls whether requests are held for manual review."""

    def __init__(self):
        self._active = False
        self._lock   = threading.Lock()
        # Queued: {id: (request, release_event, modified_holder)}
    ...
    def enqueue(self, entry_id: int, request: HttpRequest) -> HttpRequest:
        """Hold request until UI forwards or drops it. Returns (possibly modified) request."""
        ...
        event.wait(timeout=120)   # auto-forward after 2 min if UI does nothing
        with self._qlock:
            self._queue.pop(entry_id, None)
        return modified[0]   # None means drop
...
class ProxyHandler(threading.Thread):
        ...
        try:
            while True:
                ...
                if not chunk:
                    break
                data += chunk
                # Stop when we have a complete HTTP message (headers + content-length)
                if b"\r\n\r\n" in data:
    ...
    def _handle_http(self, raw: bytes):
        ...
        # Determine host/port from Host header or absolute-form request line
        ...
        # Strip absolute URL to relative path (proxy gets "GET http://host/path")
    ...
    def _handle_https(self, raw: bytes):
        ...
        # Wrap client in SSL using our per-domain cert
        ...
        # Force HTTP/1.1 — reject h2 so Chrome doesn't send HTTP/2 frames
        try:
            ctx.set_alpn_protocols(["http/1.1"])
        except Exception:
            pass

        # Reset timeout before SSL handshake
    ...
    def _forward(host: str, port: int, raw: bytes, use_ssl: bool) -> bytes:
        try:
            ...
            if use_ssl:
                ...
                # Prefer HTTP/1.1 so responses are plain text we can parse
                try:
                    ctx.set_alpn_protocols(["http/1.1"])
                except Exception:
                    pass
...
def _send_drop_response(sock):
    try:
        sock.sendall(b"HTTP/1.1 403 Dropped by BurpLite\r\nContent-Length: 0\r\n\r\n")
    except Exception:
        pass


def _error_response(msg: str) -> bytes:
    body = f"BurpLite proxy error: {msg}".encode()
    return (
...
class ProxyServer(threading.Thread):
    """Listens on a TCP port and spawns a ProxyHandler per connection."""
    ...
    def __init__(self, host: str = "127.0.0.1", port: int = 8888):
        ...
        self.running = False
        self.error:  str = ""        # set if bind fails
